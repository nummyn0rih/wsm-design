...
- **Router**: react-router-dom v6 — паттерн **`createBrowserRouter` + `<RouterProvider router={router}>`** (не `<BrowserRouter>` JSX wrapper). Конфиг роутов в `src/router.tsx`; mount в `main.tsx`.
...
// UI не вызывает save/delete; реализация выбирается при кодинге:
//   - вариант A: save/delete бросают ReadOnlyRepoError;
//   - вариант B: save/delete доступны только seed layer (internal flag).
// taraTypes.seed.ts — единственная Phase 1 точка записи в wsm:v1:taraTypes.
...
`getSnapshot`, передаваемый в `useSyncExternalStore`, **не должен возвращать новый array/object reference на каждый вызов** — иначе React уходит в бесконечный rerender (или бросает warning «getSnapshot should be cached»).

**Phase 1 pattern**: `listSync()` без `query` возвращает stable cache reference (тот же массив, пока repo не вызвал `notify`); React hook применяет filter/sort через `useMemo`:
...
function useShipments(query: ShipmentQuery) {
  ...
  const all = useSyncExternalStore(
    repo.subscribe,
    repo.listSync,        // ← stable snapshot reference, БЕЗ query
  );
  return useMemo(() => applyQuery(all, query), [all, query]);
}
```

**Запрещённый pattern** (создаёт новый array каждый вызов → infinite rerender):

```ts
// ❌ DO NOT
useSyncExternalStore(repo.subscribe, () => repo.listSync(query));
```

`listSync(query?)` сигнатура из `CrudRepo` сохраняется ради future-pattern и явных не-React вызовов; в React-хуках Phase 1 — preferred вариант без query, query-фильтрация в `useMemo`.
...
export function canEditShipmentCoreFields(role: Role, s: Shipment): boolean {
  // core: driver, tk, dates (shipDate, arrDate)
  // arrived blocks core edit; only comment editable on arrived.
  if (s.status === 'arrived') return false;
  return role === 'admin';
}

export function canEditShipmentComment(role: Role, _s: Shipment): boolean {
  // comment editable for admin + operator на ЛЮБОМ статусе (включая arrived).
  return role === 'admin' || role === 'operator';
}

export function canEditShipmentItems(role: Role, s: Shipment): boolean {
  // items (rawId, kg, supplierId, add/remove): arrived blocks item edit.
  if (s.status === 'arrived') return false;
  return role === 'admin';
}
...
- При переключении view week сохраняется (Plan на неделе X → Heatmap → Heatmap открывается на той же неделе X).
...
wsm-design/                       <repo-root>
  ...
  src/
    ...
    lib/
      ...
      raw-colors.ts               rawColor(rawId, raws) helper — READS from RawRepo;
                                  не source-of-truth, не дублирует seed.
                                  Запрещён локальный RAW_COLORS map.
      __tests__/
    ...
    data/seed/
      ...
      shipments.seed.ts           ~30 shipments × 3 weeks (previous/current/next
                                  relative to currentWeekId()), без Sunday arrDate
      plans.seed.ts               3 WeekPlans: previous (archive=true),
                                  current, next — relative to currentWeekId();
...
|---|---|---|
| S1 | Все 6 состояний Plan cell видны на current week | `plans.seed.ts` + `shipments.seed.ts` |
...
| S5 | Previous week archive — Plan view read-only, TableView edit работает | `plans.seed.ts` (`archive: true` для `offset=-1`) + `shipments.seed.ts` (previous-week shipments) |
| S6 | Current week (offset=0), next week (offset=+1) — оба editable | `plans.seed.ts` |
| S7 | Heatmap auto-filter: подмножество raw'ов в shipments < `visibleRaws` (на current week) | `shipments.seed.ts` + `plans.seed.ts` |
...
### `seedIfEmpty(repos)` flow — sequential для FK

```ts
async function seedIfEmpty(repos: Repos) {
  if (localStorage.getItem('wsm:v1:_seeded') === '1') return;

  // Sequential — порядок важен для FK consistency.

  // 1) TaraType — required by RawMaterial.allowedTara
  await seedTaraTypes(repos.taraTypes);

  // 2) Reference dictionaries (друг от друга не зависят)
  await seedRaws(repos.raws);          // depends on taraTypes (allowedTara FK)
  await seedTks(repos.tks);
  await seedSuppliers(repos.suppliers);

  // 3) Drivers — depends on TKs
  await seedDrivers(repos.drivers);

  // 4) Shipments — depends on drivers, raws, suppliers, tks
  await seedShipments(repos.shipments);

  // 5) Plans — depends on raws (visibleRaws FK + plan keys)
...
Seed создаёт 3 недели **relative `currentWeekId()`**:

- **previous** (`offset = -1`): `archive: true`.
- **current** (`offset = 0`): `archive: false`.
- **next** (`offset = +1`): `archive: false`.

Конкретные week-номера/year вычисляются в момент seed; W16/W17/W18 2025 — иллюстративный пример, не обязательные labels.

### Archive seed bypass

Runtime `PlanRepo.save(wp)` бросает ошибку для `wp.archive === true` (инвариант I9 — защита от user/service edit). Initial seed создаёт archived `WeekPlan` напрямую — это не runtime user/service edit.

Реализация выбирается при кодинге (документ фиксирует требование, не имплементацию):

- **Вариант A**: seed layer записывает в `LocalStorageRepo` через internal/protected write method, минуя archive check.
- **Вариант B**: `PlanRepo.save(wp, { allowArchive: true })` с явным флагом, доступным только seed layer.

Принципиально: runtime UI/service не должен иметь доступ к этому пути. Тесты проверяют, что обычный `PlanRepo.save(archivedWp)` бросает.
